...
This saves a working version of the texts to prevent constant processing of the same texts. 

```{r export_tidy_string}
#| eval: false
#| include: false
write_csv(all_works_tidy_string_original,"temporary_text/all_works_tidy_string_original.csv")
```

This is where the code actually starts
```{r import_tidy_string}
all_works_original_import <- read_csv("temporary_text/all_works_tidy_string_original.csv")
```
...
corpus_percent_novel_short_story <- all_works_original_import %>%
  group_by(type) %>%
...
```{r dy_corpus_size}
corpus_percent_dy_nondy <- all_works_original_import %>%
...
```{r corpus_percent_export}

write_csv(corpus_percent_all,"processed_data/corpus_percent_all.csv")
...
```{r all_works_punctuation}
write_csv(all_works_punctuation,"processed_data/all_works_punctuation.csv")
```
...
The following stats are not all that relevant

```{r punctation_abbreviation}
all_works_punctuation %>%
...
summary_by_type <- all_works_punctuation %>% 
                        group_by(type) %>% 
                        summarise(across(string_length:parenthesis, ~mean(.x)))
```
...
```{r summary_by_date}
summary_by_date <- all_works_punctuation %>% 
                    group_by(date,type) %>% 
                        summarise(across(string_length:parenthesis, ~mean(.x)))
```
