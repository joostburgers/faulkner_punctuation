...
secondary_works_abbreviations <- secondary_works_clean %>% 
  ...
  mutate(cleaned = str_replace_all (cleaned, "\\.\\.\\.", "punctellipse ")) %>%
  mutate(cleaned = str_replace_all (cleaned, "\\.\\s\\.\\s\\.\\s", "punctellipse "))
  mutate(cleaned = str_replace_all (cleaned, "\\.\\.\\.\\.", "punctellipse4 ")) %>%
  mutate(cleaned = str_replace_all (cleaned, "\\.\\s\\.\\s\\.\\s.\\s", "punctellipse4 "))
```
...
works_length_noabbreviation <- all_works_noabbreviation_type %>%
  ...
  mutate(ellipse4 = str_count(sentence, "punctellipse4"))  %>%
...
```{r ellipse_distribution}
ellipse_shapiro <- shapiro.test(log1p(summary_punctuation$ellipse))
...
```{r}
colon_shapiro <- shapiro.test(log1p(summary_punctuation$colon))
...
The preceding discusion about sentence length raises an obvious question, What is a long sentence? Is it any sentence above the average in a corpus? If so, how much longer does it need to be to be consider long and not simply longer than usual? At what point, does a long sentence cease being a sentence and instead becomes a string of unpunctuated words? James Joyce famously wrote "the longest sentence" in English in the Penelope chapter of *Ulysses*, but this sentence is not long in the same way a sentence in Faulkner is long. While there is certainly semantic coherence underlying Joyce's string of words, it does not syntactically cohere due to a lack of mid-sentence punctuation. This was rather point, of course, but it is an important distinction to make. Faulkner not only creates long strings of words, but also coordinates and subordinates his clauses through punctuation. This is quite distinct from Joyce. whereas he saw the intervention of punctuation as infelicity to the ineluctable flow of meaning making through language, the sheer profusion of punctuation in Faulkner speaks to the endless way in which meaning splits, divaricates, rejoins and ends in an arbitrary terminus only to continue again. To use two spatial metaphors, The Penelope chapter is a stream, Faulkner's writing is a watershed.  

To understand how Faulkner lengthens his sentence it is useful to look at mid-sentence punctuation. While sentence length can be calculated by measuring the distance between the first capital letter and full-stop, it is possible to get a sense of how the sentence diverts, tarries, and pauses by looking at mid-sentence punctuation like: colons, semi-colons, and parenthesis. Faulkner is not merely making sentences very long, but using punctuation to stretch out the definition of a sentence, and, in turn, call attention to the punctuation itself. The finality of the period is replaced with the indeterminacy of the unfinished sentence. What is telling about Faulkner's writing is that he does not simply write long sentences throughout his career, but experiments with different punctuation configurations to make this possible. Arguably, there are any number of texts to would form good examples, but there are three texts that deviate from Faulkner's own writing statistically: Mosquitos, Absalom, and Intruder in the Dust. 

To arrive at these outliers a statistical model of Faulkner's short stories and novels was created. These texts were broken down into sentences and each mid-sentence punctuation mark was counted. Arguably, sentence length and mid-sentence punctuation are postively correlated. As sentences get longer, there is a higher likelihood of finding mid-sentence punctuation. There are some functional limits to this correlation. Modern punctuation tends to function as a syntactical system. There are specific rules that regulate when certain punctuation marks can be used (@Shou 198). As opposed to older forms of punctuation, there tends to be an upper limit to how many times a colon, semi-colon, or parenthesis can appear in a sentence before it starts to violate the basic rules of grammar. For example, one line in *The Mansion* (DATE) has 9 semi-colons and is over 400 words long. It pushes the limits as to how many sub-clauses a reader can reasonably be expected to understand in the same coherent thought. By taking the averages of the number of mid-sentence punctuation marks across all texts, it is possible to look at their distribution. In general, these follow a right-skewed distribution; a bell curve with a long tail. Syntactically, this makes sense. If a sentence contains one colon, it is far less likely that it would also contain a second colon. This likelihood tends to decrease logirithmically, hence the very long tail on the distribution. In order to determine outliers, the log normal value of each variable was taken and plotted on a quintile-quintile plot to test for logarithimic normalcy. With this established, two different outlier tests were run: Rosner's and Adjusted box plot. Each test revealed slightly different outliers. The texts that had multiple outliers stood out. Without going into too many technical details it is safe to say that these statistical tests confirmed what many scholars had already intuited through close reading that the punctuation is odd.
