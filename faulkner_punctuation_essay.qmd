...
It does not require computational analysis to understand that Faulkner writes in long sentences. For anyone who has ever taught Faulkner, it is perhaps the most common stumbling block among students. Faulkner experienced this himself during the series of lectures he gave at the University of Virginia, no fewer than x participants asked why he wrote with such long sentences (CITE). Quantifying what readers might mean when they say Faulkner's sentences are long is a bit more complex. On average, Faulkner's sentences are only slightly longer at 17 words per sentence that the 16 words per sentence in the Corpus of English Novels (Ihrmark 79). Nor is the high sentence length a feature of all his works. The *Sound and the Fury* is notoriously complex. Nevertheless, at only 10 words per sentence, it uses fewer words per sentence than Hemingway's *The Old Man and the Sea*, which averages 13 (Ihrmark 82). Yet, there is no world in which Faulkner's more verbose style would be confused with Hemingway's trademark concision. In part this also has to do with the internal distribution of sentence lengths within each text. The *The Sound and the Fury* may have a lower average than *The Old Man and the Sea*, but it also contains one sentence that counts over 4,000 words. Clearly, this is less succinct than Hemingway, and probably most other authors as well.

A better indicator for understanding lengthiness, such as it is, is the internal structure of a sentence.

```{r}
percent_long <- works_punctuation_abbreviation %>% 
  filter(code=="SF") %>% 
  mutate(total_sentence = nrow(works_length_noabbreviation %>% filter(code=="SF"))) %>% 
  filter(string_length>17)
...
```{r}
all_outliers <- punctuation_results %>% 
  rowwise() %>% 
