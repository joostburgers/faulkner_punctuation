...
```{r correlations_string_length_rcg_all}
#Any correlations with string length that are over .2 or below .2

correlations_string_length_rcg_all <-
  demographics_punctuation_raceclassgender %>%
  ungroup() %>%
  summarise(across(where((is.numeric)),  ~ cor(.x, string_length, method =
                                                 "pearson"))) %>%
...
```{r}
target_column = "string_length"


#stuck on this problem can't figure out how to create a correlation table. 
cor_results <- demographics_punctuation_raceclassgender %>%
  ungroup() %>% 
  select(-target_column) %>% 
  select(11:50) %>% # Exclude the target column from the analysis
  summarise_all(list(cor = ~ cor(., demographics_punctuation_raceclassgender[[target_column]]))) %>% 
  gather(key = "variable", value = "correlation") %>% 
  mutate(p_value = across(where(is.numeric(),~ cor.test(demographics_punctuation_raceclassgender[[target_column]], demographics_punctuation_raceclassgender[[.]], method = "pearson")$p.value)))

```


```{r correlations_string_length_rcg_novel}
correlations_string_length_rcg_novel <-
  demographics_punctuation_raceclassgender %>%
  filter(type =="novel") %>% 
  ungroup() %>%
  summarise(across(where((is.numeric)),  ~ cor(.x, string_length, method =
...
```
```{r correlations_string_length_rcg_short_story}
correlations_string_length_rcg_short_story <-
  demographics_punctuation_raceclassgender %>%
  filter(type =="short_story") %>% 
  ungroup() %>%
  summarise(across(where((is.numeric)),  ~ cor(.x, string_length, method =
...
These demographic distributions per work can be matched to the punctuation distributions. By combining these data models, it is possible to see if there is a correlation between sentence length and the types of characters that occur. To do this, a Pearson correlation was run between 
