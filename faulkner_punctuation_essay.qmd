...
In the earliest reviews, William Faulkner's (1897--1962) style was divisive: high art for some, bad writing for others. This friction has created a rich body of scholarship on the form of Faulkner's writing generally and his use of punctuation-marks in particular. What has eluded critics, however, is a view of his writing as a whole. New computational techniques make it possible to sketch a more detailed portrait of his use of punctuation throughout his career.[^1] This data can be coupled with the character, location, and event data available in the *Digital Yoknapatawpha* (*DY*) database. Combined, this data can be used to understand not only how often Faulkner makes particular choices about punctuation-marks, but also the relationship between those choices and the characters present or mentioned in those particular sentences. Doing this confirms suspicions about specific punctuation patterns in Faulkner by earlier scholars, and furthers the conversation about who gets represented in Faulkner and how.
...
Later critics have subsumed the aesthetic of the long sentence into a general discussion of Faulkner's style. In 'Faulkner's Formalisms' Sebastian Fanzun provides a nuanced and comprehensive overview of this genealogy. Fanzun recognizes a basic polarity in approaches between the New Criticism exemplified by Cleanth Brooks (1906--1994) that tries to unify the texts into coherence, and a post-structuralist turn initiated by John T. Matthews's (? -- Present) highly influential *The Play of Faulkner's Language*, which argues for the impossibility of such an "organic unity".[@fanzunFaulknerFormalism2022 p. 23] The disunity in Faulkner's style Matthews that introduces in part helped create the opportunity for different perspectives within Faulkner to emerge. There remains, though, a lingering duality in the relationship between the radical style and the conservative politics of Faulkner's works. On the one hand, language in Faulkner has "the power to say everything" [@lockyerOrderedWordsLanguage1991 p. 68], is a "regenerative force" [@olsenRavelingOutLooping2010 p. 110], and, in the case of Rosa Coldfield, is filled with "queer potentiality". [@thomasQueerFormalismSynesthetic2016 p. 49] On the other hand, the language is fundamentally limited and limiting, where Faulkner's ways of writing about a Black character, Lucas Beauchamp, are "his ways of speaking his own racial identity", [@weinsteinFaulknerSubjectCosmos1992 p. 67] that he exercises a "politics of quietness" in *Light in August* that unifies his "racism and anticommunism",[@meyersonPulpFictionAesthetics2008 p. 39] and that his representations of Native Americans echo his own and his culture's "anxieties about white masculinity". [@parkerRedSlippersCottonmouth2002 p. 96] Computational analysis cuts across these tensions by demonstrating the simultaneous presence of continuous formal experimentation that resists a unified view of Faulkner's writing and, indeed, coherence at the sentence level, while also highlighting the unifying vision that underpins all of this experimentation.

## Computing Faulkner's Corpus

Computational analysis of Faulkner's writing presents some thorny issues that are actually quite productive when considering his use of punctuation-marks. The most significant issue is that there is not single, settled upon edition of his texts. Noel Polk (1943--2012) famously edited "corrected" versions, but even he admits they are far from the "definitive".[@polkChildrenDarkHouse1998 p. 6] In part, this is because Faulkner was inconsistent with his approach to punctuation-marks. A salient example is the Benjy section in *The Sound and the Fury*, which Faulkner retyped three times using a different punctuation system.[@polkChildrenDarkHouse1998 p. 14] These permutations are evidence of Faulkner's constant experimentation with punctuation and his style more generally throughout his career. Still, Polk's own modesty about his accomplishment notwithstanding, this paper uses his "corrected" editions for the corpus where possible as these also align with the texts used for creating the *Digital Yoknapatawpha* database.

Beyond the lack of a "definitive" edition another challenge for computational analysis is the internal coherence of the textual corpus. Faulkner's writing is very heterogeneous, consisting of novels, short stories, poetry, film scripts, and a text that hovers between a stage play and a novel: *Requiem for a Nun*. As these texts all represent different types of writing, only novels and short stories were considered. Of these, *Requiem for a Nun* and the *Appendix: Compson* were excluded as well, because they used radically different formatting from all of Faulkner's other prose.

Furthermore, Faulkner's corpus also includes a lot of overlap due to intertextuality. There are the 14 novels and 54 short-stories that take place in his apocryphal Yoknapatawpha County. Of these short stories, many of which were originally published in popular magazines, about 5% was incorporated wholesale or in part into novels such as *The Unvanquished* (1938), *The Hamlet* (1940), and *Go Down, Moses* (1942), resulting in .4% of the text being duplicated verbatim across the corpus. This intertextuality does not follow a neat timeline. Some works written early in his career are only published much later, like "Afternoon of a Cow" which was published forty years after he first sketched out the idea.[@faulknerUncollectedStoriesWilliam1997 p. 703] Meanwhile, two substantially different versions of 'The Bear', one in The *Saturday Evening Post* the other in *Go Down, Moses,* are published within days of each other.[@corriganFaulknerBear2015] This makes it difficult to establish a concrete timeline of his stylistic development. Since the composition date and publication date can vary widely, patterns that appear in one text may, in a certain sense, *belong* to a previous text. This makes it very difficult to detect if there is any gradual, unidirectional change in style as is possible with some other authors, like Henry James.[@hooverModesCompositionHenry2014 p.275]
...
```{r sound_fury_distribution}


sf_median_mean <- all_works_punctuation %>% 
                  filter(code=="SF") %>% 
                  summarise( mean=mean(string_length), median= median(string_length), skew = skewness(string_length))


```

Going by the imperfect measure of sentence length, Faulkner's writing is not exceptional compared to other writing at the time. At `r round(corpus_mean_string_length,2)` Words-Per-Sentence (WPS), Faulkner's sentences are only slightly longer than the average 16 WPS for a text in the Corpus of English Novels, which covers writing from 1881 - 1922 [@ihrmarkCorpusStylisticAnalysis2021 p. 79] and longer still than the 13 WPS around the 1930s.[@RN37 p. 233] Nor is the slightly higher than average sentence length a feature of all his works. The *Sound and the Fury* is notoriously complex. Nevertheless, at only `r round(average_sf,0)` WPS, it uses fewer words on average *The Old Man and the Sea* (13 WPS) by Ernest Hemingway (1899-1961).[@ihrmarkCorpusStylisticAnalysis2021 p. 82] Despite this counter-intuitive disparity, Faulkner's more verbose style could never be confused with Hemingway's trademark concision. In part, this has to do with the distribution of sentence lengths within each text. *The Sound and the Fury* may use fewer words-per-sentence on average than *The Old Man and the Sea*, but it also contains one sentence that counts over 4,000 words. Clearly, this is less succinct than Hemingway, and probably most other authors as well. These long sentences, it should be noted, are more of a statistical exception than the rule. The normal distribution for *The Sound and the Fury*, and most of Faulkner's other texts is heavily right-skewed (mean = `r round(sf_median_mean$mean[1],2)`, median = `r round(sf_median_mean$median[1],2)`, skewness = `r round(sf_median_mean$skew[1],2)`).[^3] Meaning that an overwhelming majority of sentences are of average length, but a few are much, much longer. In the case of the *Sound and the Fury* only about `r percent(sf_percent_long)` of the sentences are longer than the mean corpus sentence length. The same can be said for Faulkner's other texts, most of these (`r percent(1-sentence_length_percentage)`) have an average sentence length lower than the total average across the corpus. More succinctly, a small share of sentences in each work gives the impression of long sentences, and, similarly, within the corpus of Faulkner's novels and short stories there are a minority of works that are above average in sentence length. Statistically, lengthy sentences are a local feature not a global phenomenon.

[^3]: The difference between the median and the mean indicates that there are several high values, long sentences, that are increasing the average. Meanwhile, the skewness measures the magnitude of this effect. Any number above or below 0 indicates asymmetry in the normal distribution. In this case, the skewness is very high.

Nevertheless, lengthy sentences are seen as the hallmark of Faulkner's writing. In part this may be because some of Faulkner's most canonical works feature long sentences, *Go Down, Moses* (`r round(average_gdm,0)`), *Absalom, Absalom* (`r round(average_aa,0)`), 'A Rose for Emily' (`r round(average_re,0)`), and 'Barn Burning' (`r round(average_bb,0)`) These works are, in a sense, more Faulknerian than a relatively unknown early text like 'Elly' (`r round(average_el,0)`). Meanwhile, equally canonical works like 'That Evening Sun' (`r round(average_tes,0)`), *Light in August* (`r round(average_lia,0)`) , and *As I Lay Dying* (`r round(average_aid,0)`) do not, on average, stand out as being particularly verbose relative to the the general corpus of texts written in English during this period. Despite these counter indications, it goes against a firmly established scholarly tradition based on Faulkner's own statements and even common sense, to argue that lengthy sentences are not Faulkner's particular métier. Instead, to understand why Faulkner's sentences, for lack of a better word, *feel* long it is informative to look at their internal structure.

## Extending the Sentence: Faulkner's Experiments in Medial Punctuation

The preceding discussion about sentence length raises an obvious question: What is a long sentence? Is it any sentence above the average sentence length of the corpus? Is there a minimum threshold at which sentences cease to be longer than usual and become "long"? If a sentence is indeed "long", is there a maximum length? At what point, does a string of words grammatically cease to be sentence? These questions can be answered, in part, by drawing a contrast with another modernist who not only inspired Faulkner but was also famous for his long sentences: James Joyce. Joyce wrote some of the longest sentences in the English language in the Molly episode of *Ulysses,* but they are not long in the same way a sentence in Faulkner is long. [@RN35 p. 102] The Molly episode is essentially a string of syntactically coherent sentences with the punctuation removed.[@attridgeMollyFlowWriting1989 p. 545] Faulkner not only creates long strings of words, but they are rendered syntactically unconventional and challenging through his use of punctuation-marks. Thus, whereas Joyce saw the intervention of punctuation-marks as an infelicity to the ineluctable flow of language in Molly's soliloquy,[^4] the sheer profusion of punctuation-marks in Faulkner speaks to the endless way in which meaning branches, eddies, converges, and cascades over the terminal punctuation-mark only to recirculate again. Molly's soliloquy is a stream, Faulkner's writing is a watershed.
...
A statistical model of the punctuation in his short stories ( $n$ = `r text_type_count$n[2]`) and novels ( $n$ = `r text_type_count$n[1]`) can help determine which texts use punctuation-marks at variance with the corpus more generally.[^5] The texts were broken down into sentences, and each medial punctuation-mark was counted. Intuitively, sentence length and medial punctuation-mark are positively correlated. As sentences get longer, there is a higher likelihood of finding medial punctuation-mark. There are some functional limits to this correlation, however. Modern punctuation tends to function as a syntactical system that regulates the number of certain punctuation marks that can be used.[@schouSyntacticStatusEnglish2007 p. 198] Hence, when one sentence in *A Fable* (1954) has 53 commas, 9 semi-colons, 6 colons, 2 statements surrounded by lunalae, and is over 750 words long, it disrupts conventional understanding of how those punctuation marks should be used.[@faulknerFable2011 pp. 253-255]
...
By taking the averages of the number of medial punctuation-marks per sentence across all texts, it is possible to detect if a text deviates from the overall pattern within Faulkner's corpus. Intuitively Faulkner's punctuation-mark patterns are different from other writing and comparing his work to other writing such as "modernism" or "standard English" is not techincally very involved, but raises larger issues about what those terms actually mean. That said, phonological and syntactical phenomena tend to follow a power-law distribution; where the probability decreases exponentially with each frequency interval.[@newmanPowerLawsPareto2005 p. 372; @sunFrequencyDistributionsPunctuation2018 p. 32] Syntactically, this makes sense. In any given text, most sentences do not contain commas. If a sentence does contain a comma, it is far less likely that it would also contain a second comma, it is exponentially less likely to contain a third comma, and so forth.[^6] The curve for these distributions looks a lot like a very steep cliff protruding from a plain; the line drops from the y axis into a sharp turn to the right where it meets the x-axis and extends in a long tail. If Faulkner did not experiment, a similar distribution would be present, and there would be little variance in the number of punctuation marks. This is not borne out by the data. Plotting the distributions across all punctuation marks reveals an exponential distribution best described as chi-squared with three degrees of freedom ($\chi^2_3$). Visually, this curve looks a bit like a slide on a playground: a steep ladder on the left-hand side with a long-curve that never quite reaches the ground on the right. This curve indicates that many texts use the same punctuation-marks similarly, but a few have a very high relative frequency of specific punctuation marks.

[^6]: In theory, this rule does not necessarily to specific punctuation marks that are more strictly governed by the rules of grammar, such as colons, which generally do not exceed one per sentence.

Determining which values are outside the expected range and therefore outliers is a matter of some interpretation. A variety of tests revealed that for each medial punctuation-mark different texts were outliers, which suggests an inconsistent use of medial punctuation-marks.[^7] This variance does not appear to be strongly time dependent. There were no distinct phases to his punctuation-mark experiments, and he does not, for example, use lunulae heavily in the beginning of his career and semi-colons more extensively later in his career. Quite the opposite is true. Individual works stand out as having a substantially higher frequency of certain medial punctuation-marks in comparison to others, and punctuation-mark patterns that are the hallmark of some works are entirely absent in others. This difference matters because medial punctuation-marks are Faulkner's primary vehicle for creating long sentences, and function as a statistical signature for particular works. Three texts provide useful insights. *Absalom, Absalom*, and *Intruder in the Dust*, and 'Miss Zilphia Gant' were all marked as statistical outliers.

[^7]: Deciding that a data point is an outlier is an ongoing debate among mathematicians,[@hawkinsIdentificationOutliers1980 pp. 9-12] and, ultimately, the decision to say that a data point is outside the expected range is still interpretive. That said, two different outlier tests were were run: Rosner's multiple outlier test [@millardEnvStatsPackageEnvironmental2013 p. 1106] and and the Adjusted Box plot method.[@hubertAdjustedBoxplotSkewed2008] All of the calculations and results for these tests are available here:

As one of Faulkner's most challenging texts, it is unsurprising that the punctuation-mark patterns in *Absalom, Absalom!* diverge from the corpus. Averaging around 43 WPS, the text is notable for its prolific use of parentheses. With this technique, sentences are interrupted by parenthetical statements leading to sentences of extraordinary length. In one sentence stretching over 1,000 words, Faulkner uses nine statements surrounded by lunulae as a running commentary on the main sentence.[@faulknerAbsalomAbsalomCorrected2011 pp. 148-50]Some of these parenthetical statements are themselves several lines. All of this is complicated by the fact that this 1,000 word sentence appears in Chapter 6. The majority of this chapter is one long parenthetical statement. As a result, the parentheses in this sentence are actually nested within surrounding parentheses. Functionally, the sentence is an aside to an aside. Beyond this particular sentence, the nesting continues throughout chapter six and at some point the the word Quentin is nested four levels deep.[@faulknerAbsalomAbsalomCorrected2011 p. 106]That is, as an aside to an aside to an aside. There are so many lunulae in *Absalom, Absalom!* that it appears that Faulkner or possibly his editor lost track of them, because the chapter has an uneven number of parentheses and is missing a final closing parenthesis. Not only are Faulkner's sentences very long in *Absalom, Absalom!*, they are also, in a sense, deep. Each parenthetical statement adds more commentary and context, which itself requires more commentary and context. Therefore, it is fitting that peeling away all of these parentheses like layers of an onion reveals the novel's putative narrator: Quentin. The tale is ultimately about the teller.

In a slightly different fashion, Faulkner instrumentalizes colons in *Intruder in the Dust* to create sentence depth. To give an extreme example, one sentence in *Intruder in the Dust* is over 1,600 words long and features 18 colons that function as clausal adjuncts. Geoffrey Nunberg (1945--2020) labels this phenomenon colon-expansion, whereby the content following the colon expands or elaborates on the preceding clause.[@nunbergLinguisticsPunctuation1990 p.30] Importantly, he points out that there are only two constraints to a colon-expansion. First, there is a semantic limit to the extent to which something can be elaborated, and, two, colon-expansions cannot themselves contain other colon-expansions.[@nunbergLinguisticsPunctuation1990 pp. 30-31] Faulkner violates the former by dint of the latter. Each sentence dilates in meaning as he adds on more clausal adjuncts and elaborations. While it is impractical to quote Faulkner's multi-page sentence in its entirety, the opening sequence captures this effect: "Because he was free: in bed: in the cool familiar room \[...\]".[@faulknerIntruderDust2011 p. 34] With each colon the narrative situation becomes more complex. The choice for concatenating clauses with a colon and not simply writing new sentences, appears to be similar to the depth effect created through lunulae in *Absalom, Absalom!*. With each new colon the reader is tasked with processing a new piece of information that falls within the scope of one sentence. The narrative does not move forward but instead moves deeper.

It does not require inferential statistics to understand that *Absalom, Absalom!* and *Intruder in the Dust* have unusual punctuation patterns. One surprising statistical outlier is 'Miss Zilphia Gant', which has the highest relative frequency of ellipses in the corpus. It sits in the middle of a cluster of stories from 1927-1934 that average a higher number of ellipses per sentence than other Faulkner texts. To mark this as a general feature of Faulkner's style during this period would be a mistake, however; as canonical works like *The Sound and the Fury*, *As I Lay Dying*, and *Light in August*, all written around this time, do not feature nearly as many ellipses. This was simply one of many punctuation-marks he was experimenting with in some in some specific contexts, the most example of which is 'Miss Zilphia Gant'. In this relatively obscure story, Miss Zilphia grows up a virtual captive of her overbearing mother. What makes the story unique is that its use of three dots is two-fold. Sometimes it is used in the modern form of the ellipse as a deferral of meaning,[@tonerEllipsisEnglishLiterature2015 p. 151] at other times they function more like dashes that enclose an apposition. [@railtonManuscriptsMissZilphia2017] To complicate matters, in the early drafts, Faulkner, conversely, uses dashes for their commonly understood use as "dash-interpolations",[@nunbergLinguisticsPunctuation1990 pp. 26-27] but also as ellipses. The effect is that the punctuation-marks become a site of elision as well an attenuation that creates a myriad of possibilities for the unsaid. For example, after Miss Zilphia finally marries a man, she still feels the urge to return to her mother. In the early typescript, her husband pleads with her, "if you go now ------ Zilphy".[@faulknerMissZilphiaGant] In the published version this becomes "If you go in there . . . Zilphy". [@faulknerUncollectedStoriesWilliam1997 p. 377] Stephen Railton speculates that this change was likely made by the editor of the Book Club of Texas, which first published this story.[@railtonManuscriptsMissZilphia2017] Whatever the reason for the final version, the multiple drafts all attest to the fact that Faulkner was experimenting with using entirely new patterns of punctuation-marks that could defer and displace the full meaning of the sentence to an unknowable place behind the ellipse thereby expanding its possibilities.
...
At the level of the individual text, the relative frequency of character types that occur in the text can be compared to sentence length. A powerful way of doing this is through a Pearson correlation, which produces two important numbers. First, the $p$-value indicates the chance that the relationship between the two variables is random. Generally, any result where $p<.05$ is seen as significant. In plainer English, this means that there is a less than 5% chance that the pattern is random. The second value, the correlation coefficient $r$, indicates both the direction and the magnitude of the relationship. The $r$ value of a Pearson correlation runs from -1 to 1, where a negative correlation indicates an inverse relationship and a positive correlation indicates a direct relationship. Though interpreting the magnitude of a correlation varies substantially from discipline to discipline, [@chenCorrelationParametricNonparametric2002 p. 12] the most commonly cited ranges establish any correlation between .3 and .5 or between -.3 and -.5 as a medium effect, and above .5 or below -.5 as a large effect.[@cohenStatisticalPowerAnalysis1988 pp. 79-80] The Pearson correlation between sentence length and the race, class, and gender composite suggests a medium to strong positive correlation between sentence length and texts that feature four types of characters: Enslaved women who have both Black and White ancestry ($r$ = `r round(correlation_results_all$cor[1],3)`), upper-class men who have both Black and White ancestry ($r$ = `r round(correlation_results_all$cor[4],3)`), Men who have both Black and White ancestry and whose class is indeterminable ($r$ =`r round(correlation_results_all$cor[3],3)`), and upper-class women who have both White and Black ancestry ($r$ = `r round(correlation_results_all$cor[2],3)`).[^9] Needless to say, in Faulkner's highly stratified social world, these characters are anomalous. If short stories are removed from the data set, this trend becomes even more pronounced. Indicating that this is more of a feature of the novels than the corpus as a whole. By and large, this correlation highlights that novels that deal with the transgression of the South's strict race and boundaries have a higher tendency to use long sentences: *Absalom, Absalom!* (`r round(average_aa,0)` WPS), *Intruder in the Dust* (`r round(average_id,0)` WPS), and *Go Down, Moses* (`r round(average_gdm,0)` WPS). The only notable exception is *Light in August* (`r round(average_lia,0)` WPS), which has Joe Christmas's racial ambiguity as its main theme, but does not, on the whole complement this stylistically with long sentences as expected. Another confounding feature is that the other set of texts with above average sentence lengths is the Snopes trilogy (*The Hamlet*, *The Town*, *The Mansion*), which is actually notable for being almost entirely devoid of non-White characters.[^10] This discrepancy also underscores the limitations of this analysis. After all, just because a text features more non-White characters than other texts does not mean that they are central to the action, or appear in those long sentences.

[^9]: The terms are descriptions of the concatenation of three categories class, gender, and race. In *DY,* these categories all have specific nuances documented in the instructions. Importantly, classes for White characters run from Poor White to Upper Class. For Black characters the classes are enslaved and free, because in the logic of Faulkner's Jim Crow South there is no possibility of Black characters being of an equal social standing as Whites even if they share the same economic fortunes. In terms of race, characters can either be Black, Native American, White or a combination of the three.

[^10]: Only `r percent(demographic_snopes$percent[3])` are not-White compared to `r percent(demographic_snopes$percent[1])` in the other texts.
...
In the *Digital Yoknapatawpha* database, the events data table stores the coincidence of characters and locations, where each event represent one action by a single or group of characters in one location. As the definition of an event is semantic and not syntactic, it effectively means that events are not coextensive with sentences. Occasionally, an event can run for several pages capturing multiple sentences or it can be only a few words in the middle of a sentence. Although technically involved, events can be reconciled with the sentences in which they appear and, conversely, sentences with the events in which they appear. This information can be joined with the character table to indicate what characters are present and mentioned in every sentence.[^11] This approximates of the relationship between characters and sentences. Notably, there is no way of knowing what role each character plays in the event; that data simply does not exist. It could be that a character is simply in the background and is not the main part of the action. The baseline assumption is that a frequent recurrence of a character type indicates some salience.

[^11]: The *DY* database stores the first 8-10 words of each event. This makes it possible to delimit where an event begins and ends in each part of a digital version of the text. Occassionally, this requires manual realignment due to errors in the database or errors in the text. In either case, these realignments are statistically insignificant for the overall picture of punctuation patterns.

Since there are roughly ninety-thousand sentences in the entire corpus that can be anywhere from one to six thousand words long there is a really broad range of sentence types in which characters can appear. As with the analysis by text, a Pearson correlation correlation test establishes possible relationships between the types of characters present in a sentence and sentence length.[^12] The results are at odds with those of the text-level correlation in the previous section. Whereas the presence of characters with multiple ancestries tended to be predictive of sentence length at the level of the work, at the level of the sentence there is a strong correlation between sentence length and whether an upper-class White man is present ($r$= `r round(cor_upper_class_white_male_present,3)`) or mentioned ($r$= `r round(cor_upper_class_white_male_mentioned,3)`). As upper-class White men tend to be the protagonists of these novels, it makes sense that appear in the longest sentences. What is quite telling is who is less likely to be present. It is true for both upper-class White women, lower-class White men, and free Black men that their correlation coefficient is higher when they are mentioned ($r$= `r round(cor_upper_class_white_female_mentioned,3)`, $r$= `r round( cor_lower_class_white_male_mentioned,3)`, and $r$= `r round(cor_free_black_male_mentioned,3)` respectively) than when they are present ($r$= `r round(cor_upper_class_white_female_present,3)`, $r$= `r round( cor_lower_class_white_male_present,3)`, and $r$= `r round(cor_free_black_male_present,3)` respectively). The difference between these correlations is notable, as the strength of the correlation reduces by about 15%.[^13] That said, this drop does not mean that upper-class White women, lower-class White men, and free Black men are necessarily being spoken about in these sentences. Mentioned in the database is a catch-all term for any character who is mentioned in an event but does not actually appear. It could be that the event only references a character once or that the entire event is about that character. Furthermore, correlation is not causation. If one of these characters is present it does not mean that the sentence length will get shorter, only that they are more likely to be mentioned than present in longer sentences.

[^12]: This is a very large sample size, and the $p$-value, the odds that the correlation is random, tends to be quite low.

[^13]: Correlation coefficients are not linear in strength. Thus, a coefficient of .4 is not twice as strong as on of .2. The coefficient of determination, $r^2$, explains the variance between the two relationship between the two variables.[@chenCorrelationParametricNonparametric2002 p. 12] For these numbers, the average difference in the $r^2$ values is 15%.

All those caveats aside, the correlations highlight a common scenario in Faulkner: two White men, usually upper-class, discussing another person who is either not male, upper-class, White, or or all three. This includes Quentin and Mr. Compson talking about Caddy in *The Sound and the Fury*, and Ratliff and Gavin Stevens talking about the lower-class Flem Snopes in *The* *Town* and *The* *Mansion*, and any number of White men talking about Black men. What falls away from this statistical picture at the level of the character type is the correlation between sentence length and any troubling of the racial binary of the Jim Crow South. There is only a weak correlation between mixed ancestry characters and sentence length. A plausible explanation is that since mixed ancestry characters only feature heavily in a few texts, this is not an overall pattern in the corpus for that character type.
...
correlation_lion_boon <- round(correlations_lion$value[1],3)
                          
correlation_lion_quentin <- round(correlations_lion$value[2],3)  
                          

```

```{r sentence_demographic_alonegroup_import}
...
```{r alone_in_text_white_male}
alone_in_text_white_male <- alone_in_text %>% 
                            filter(race_class_gender=="white_upper_class_male") %>% 
  pull(alone_percent)
```

To account for a low distribution of a particular character type across the corpus, character and sentence length and also be correlated within each text. The pattern that occurs is quite telling. Essentially, upper-class White men consistently have a moderate to strong correlation with longer sentences, but there are also other character types whose correlation suddenly becomes stronger in relation to sentence length. Thus, in the short story 'Lion' both upper-class White men and the lone lower-class male with Native American and White ancestry, Boon Hogganbeck, strongly correlate with longer sentences ($r$ = `r round(correlation_lion_quentin,3)`, $r$ = `r round(correlation_lion_boon,3)`). Still, this is probably because Boon happens to appear frequently in scenes with an upper-class white male, Quentin Compson. This is why the average sentence length when an upper-class White male is alone is `r round(lion_alonegroup$alone,0)` WPS, which drops to `r round(lion_alonegroup$group,0)` WPS when there are other characters present.[^14]

[^14]: It is not possible to run a correlation test on only those sentences where a character is alone, because there are too few data points. Generally, though it is true that a character alone, irrespective of race, class, or gender tends to be embedded in longer sentences than in a group. That said, the plurality of events where a character was alone featured upper-class white males (`r percent(alone_in_text_white_male)`)
...
The final and most fine-grained way to consider sentence length is by individual characters. This shows which specific characters tend to be present or mentioned in lengthy sentences. As the previous data already indicates, the types of characters who are consistently present or mentioned in long sentences is quite limited. It should therefore be little surprise that the character who has the strongest correlation with sentence length is Quentin Compson ($r$= `r quentin_correlation`), who throughout the *Sound and the Fury* and *Absalom, Absalom* is at the centre of the highly experimental narrative. Notably, Mr. Jason Compson, his father ($r$= `r mr_jason_compson_correlation`), also has a moderate correlation with sentence length when he is mentioned. The first woman on the list is Caddy Compson ($r$= `r caddy_compson_correlation`), and the first person white person who is not upper-class is the murderer Vinson Gowrie ($r$= `r vinson_gowrie_correlation`) whose mention correlates with longer sentences. Also included is the mixed ancestry Charles Bon ($r$ = `r charles_bon_correlation`) who, importantly, may be present in the event, but is constructed by a White narrator, mainly Quentin. In short, when drilling down to specific characters, it becomes clear that there is a higher tendency for them to be longer when Quentin is present.

```{r requiem_punctuation}

requiem_punctuation <- all_works_punctuation %>% 
                        filter(string_length>6000)

requiem_string_length <- requiem_punctuation %>% 
                         pull(string_length)

requiem_comma <- requiem_punctuation %>% 
                 pull(comma)

requiem_semi_colon <- requiem_punctuation %>% 
                 pull(semi_colon)

requiem_colon <- requiem_punctuation %>% 
                 pull(colon)

requiem_parenthesis <- requiem_punctuation %>% 
                 pull(parenthesis)

```

```{r demographics_longest sentence}
#Import demographics created in text_processing supplement correlations. 
demographics_longest_sentence_import <- read_csv("processed_data/demographics_longest_sentence.csv", show_col_types = FALSE)

```

```{r descriptive_stats_longest_sentence}

longest_sentence_presentmentioned <- demographics_longest_sentence_import %>% 
                                      group_by(presentmentioned) %>% 
                                      summarise(value = sum(value))

longest_sentence_distinct <- demographics_longest_sentence_import %>% 
                              distinct(name) %>% 
                              nrow()
...
This is a change As with all statistics, some caution is always necessary when using them to make generalizations, especially when it comes to literature where so often it is the exception that is the richest site for interpretation. The central text that has been excluded from this analysis is *Requiem for a Nun*, which also includes Faulkner's longest sentence by some distance: `r requiem_string_length`. Its punctuation-marks include `r requiem_comma` commas, `r requiem_colon` colons, `r requiem_semi_colon` semi-colons, `r requiem_parenthesis` parentheses, and one full-stop. It traces the westward march of the settlers of Yoknapatawpha from 1797 to 1951, and features `r longest_sentence_presentmentioned$value[1]+ longest_sentence_presentmentioned$value[2]` distinct characters -- `r longest_sentence_presentmentioned$value[1]` of whom are present and `r longest_sentence_presentmentioned$value[2]`of whom are mentioned -- which can be split into `r longest_sentence_distinct` character types. It captures the past, present, and possible future of not only one man or woman, but all of Yoknapatawpha's men and women. The sheer, dizzying profusion of words radically challenges the way in which punctuation and even language is meant to be understood. It is, *sui generis*, Faulkner's fullest incarnation of the long sentence.
