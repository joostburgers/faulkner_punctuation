...
These demographic distributions per work can be matched to the punctuation distributions. At the level of the individual text, the relative frequency of character types that occur in the text can be compared to sentence length. A powerful way of doing this is through a Pearson correlation, which tests whether two numbers rise or fall together, the correlation coefficienct $r$, and whether this pattern is not random, the significance $p$.[^8] Unless otherwise indicated, this paper adopts the standard definition of significance at $p<.05$, which, expressed in plainer English, means that there is a less than 5% chance that the pattern is random. The magnitude of a Pearson correlation runs from -1 to 1, where a negative correlation indicates an inverse relationship and a positive correlation indicates a direct relationship. Though interpreting correlation varies substantially from discipline to discipline, [@chenCorrelationParametricNonparametric2002 p. 12], the most commonly cited ranges establish any correlation between .3 and .5 or between -.3 and -.5 as a medium effect, and above .5 or below -.5 as a large effect.[@cohenStatisticalPowerAnalysis1988a pp. 79-80] The Pearson correlation between sentence length and the race, class, and gender composite suggests a medium to strong positive correlation between sentence length and four types of characters: Enslaved women who have both Black and White ancestry ($r$ = `r round(correlation_results_all$cor[1],3)`), upper class men who have both Black and White ancestry ($r$ = `r round(correlation_results_all$cor[4],3)`), Men who have both Black and White ancestry and whose class is indeterminable ($r$ =`r round(correlation_results_all$cor[3],3)`), and upper class women who have both White and Black ancestry ($r$ = `r round(correlation_results_all$cor[2],3)`). Needless to say, in Faulkner's highly stratified social world, these characters are anomalous. If short stories are removed from the data set, this trend becomes even more pronounced. Indicating that this is more of a feature of the novels than the corpus as a whole. By and large, this correlation highlights that novels that deal with the transgression of the South's strict race and boundaries have a higher tendency to use long sentences: *Absalom, Absalom* (`r round(average_aa,0)` WPS) , *Intruder in the Dust* (`r round(average_id,0)`WPS), and *Go Down, Moses* (`r round(average_gdm,0)`WPS). The only notable exception is *Light in August* (`r round(average_lia,0)`WPS), which has Joe Christmas's racial ambiguity as its main theme, but does not, on the whole complement this stylistically with long sentences as expected. Another confounding feature is that the other set of texts with above average sentence lengths is the Snopes trilogy (*The Hamlet*, *The Town*, *The Mansion*), which is actually notable for being almost entirely devoid of non-White characters.[^9] This discrepancy also underscores the limitations of this analysis. After all, just because a text features more non-White characters than other texts does not mean that they are central to the action, or appear in those long sentences.

[^8]: Pearson correlations produce two important numbers significance ($p$) and the correlation coefficient ($r$). Significance indicates the percent chance that the relationship is not random. Unless otherwise indicated the threshold for significance was set at $p$\<.05.
...
[^12]: Correlation coefficients are not linear in strength. Thus, a coefficient of .4 is not twice as strong as on of .2. The coefficient of determination, $r^2$, explains the variance between the two relationship between the two variables.[@chenCorrelationParametricNonparametric2002 p. 12] For these numbers, the average difference in the $r^2$ values is 15%.
