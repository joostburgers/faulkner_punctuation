...
#create a co-occurrence matrix of each character in each event
...
Using these two data models, that of all the punctuation, and that of all the characters, it is possible to get an understanding of the relationships between characters and punctuation. As with all such inferential models, the result are very much dependent on the underlying assumptions about the data. Suffice to say that there is not one easy test to run, and that each sheds light on possible relationships, but there is not one single test that provides a tidy answer. Given this, relationships between characters and punctuation can be imagined at three different scales: book, type, and name. In the first, the total character composition of the book is taken and compared to sentence lenght, in the second the character types in each event is compared to sentence length, and in the third each individual character is taken to see if there is a relationship. As the tests get more granular, there are more data points, and generally, there is a higher confidence that the pattern is not random.
