...
#| eval: false
#| include: false
write_csv(all_works_tidy_string,"temporary_text/all_works_tidy_string.csv")

```

```{r}
all_works_tidy_string_import <- read_csv("temporary_text/all_works_tidy_string.csv")
```
...
characters <- read_csv("dy_database/characters.csv")
locations <- read_csv("dy_database/locations.csv")
events <- read_csv("dy_database/events.csv")
```
...
dy_events_data <- read_csv("processed_data/dy_database_flattened_2024_1_21.csv") %>% 
...
correlation_matrix_present_work <- sentence_demographics_present %>%
  select(code, string_length, all_of(present_correlation_matrix_remove)) %>%
  group_by(code) %>%
  replace(is.na(.), 0) %>% 
...
correlation_matrix_long_present <- correlation_matrix %>%
