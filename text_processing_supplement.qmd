...
all_work_correlation <- correlation_work_both_result %>% 
                        bind_rows(correlation_work_mentioned_result) %>% 
                        bind_rows(correlation_work_present_result) %>% 
...
correlation_names_work_present_result <- correlation_names_work %>%
  pivot_longer(cols = starts_with("all_"),
               names_to = "name") %>% 
                drop_na() %>% 
               mutate(correlation_cof= ifelse(str_detect(name, "_cor"),value,NA)) %>% 
...
#| eval: false
#| include: false
all_work_names_correlation <- correlation_names_work_present_result %>% 
                              bind_rows(correlation_names_work_mentioned_result) %>% 
 group_by(presentmentioned) %>% 
                        distinct(name,n) %>% 
                        ungroup() %>% 
                        pivot_wider(id_cols = name, names_from = presentmentioned, values_from = n) %>% 
