---
title: "Text Processing Supplement: Correlations"
format:
  html:
    theme: cosmo
    css: styles.css
...
Textual pre-processing implements some common procedures for cleaning up the text including removing abbreviations, standardizing quote marks, and, in general, removing artefacts that skew calculations.
...
```{r}
all_events_sentences <- read_csv("temporary_text/all_events_sentences.csv", show_col_types = FALSE)
```
...
correlation_work_both_result <- correlation_work_both %>%
  ...
  drop_na() %>% 
  filter(p_value<.05) %>% 
    filter(correlation_cof>.1 | correlation_cof< -.1) %>% 
  add_count(name) %>% 
...
write_csv(all_correlation, "processed_data/all_correlation_charactertype_expanded.csv")
...
```{r all_work_correlation_export_two}
...
```{r}
top_percent <- sentence_demographics_present %>% 
               summarise(top_percentile = quantile(string_length, .99))
```
...
```{r}
#This calculated the percent of sentences in the top 1% and which characters were present

top_names = all_events_sentences %>% 
    filter(string_length>top_percent$top_percentile[1]) %>% 
  left_join(names_demographic, by = "nid") %>% 
  filter(presentmentioned=="Present") %>% 
  select(starts_with("all_")) %>%
...
This data set was custom created for *Requiem for a Nun*. It looks at what characters appear in Faulkner's longest sentence. The reason for doing this manually is to verify that the correct events have been chosen.

```{r}
reqiuem_events <-  read_csv("processed_data/dy_database_flattened_2024_1_21.csv", show_col_types = FALSE) %>% 
                  rename_with( ~ tolower(str_replace_all(.x,"\\s+|\\p{Punct}", "_"))) %>% 
  filter(sourcetextcode == "RQ") %>% 
  filter(pagenumber >168 &pagenumber <206) %>% 
  select(
    sourcetexttitle,
    sourcetextcode,
    pagenumber,
...
```{r}
#wanted to see how many different locations, only 10. Not all that interesting
reqiuem_events_location <- reqiuem_events %>% 
                            distinct(location)
```


```{r}
reqiuem_events_demographics <- reqiuem_events %>% 
    mutate(race_class_gender = paste(race, class, gender, sep ="_")) %>%
  mutate(race_class_gender = tolower(str_replace_all(race_class_gender, " ", "_"))) %>%
  group_by(sourcetextcode, presentmentioned, nid) %>%
  pivot_wider(
...
```{r}
demographics_longest_sentence <- reqiuem_events_demographics %>% 
                                 group_by(presentmentioned) %>% 
                                  select(starts_with("all_")) %>%
  summarise(across(everything(), sum, na.rm = TRUE)) %>% 
                                pivot_longer(starts_with("all_")) %>% 
...
```{r}
write_csv(demographics_longest_sentence, "processed_data/demographics_longest_sentence.csv")
```



```{r}
longest_sentence_presentmentioned <- demographics_longest_sentence %>% 
                                      group_by(presentmentioned) %>% 
                                      summarise(sum(value))
```
...
```{r}
longest_sentence_distinct <- demographics_longest_sentence %>% 
                              distinct(name)
```

```{r}
longest_sentence_composition <- demographics_longest_sentence %>% 
                                group_by(presentmentioned) %>% 
                                mutate(percent = value/sum(value))
